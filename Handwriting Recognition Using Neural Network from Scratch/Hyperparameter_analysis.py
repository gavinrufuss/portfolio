...
#Visualizing model performance using plots
...
def run_experiment(hidden_units_list, learning_rate_list, num_epoch=50, init_flag=True):
    X_train, y_train, X_val, y_val = load_data_large()

    # Experiment 1: Varying Number of Hidden Units
    avg_train_entropies = []
    avg_valid_entropies = []
    for hidden_units in hidden_units_list:
        print(f"Training with {hidden_units} hidden units...")
        _, _, train_entropies, valid_entropies = SGD(X_train, y_train, X_val, y_val, hidden_units, num_epoch, init_flag, 0.01)
        
        # Early stopping condition check (if validation loss doesn't improve by 0.001)
        last_val_entropy = float('inf')
        for epoch, val_entropy in enumerate(valid_entropies):
            if last_val_entropy - val_entropy < 0.001:
                print(f"Stopping early at epoch {epoch} due to small validation loss improvement.")
                break
            last_val_entropy = val_entropy

        avg_train_entropies.append(np.mean(train_entropies[-5:]))
        avg_valid_entropies.append(np.mean(valid_entropies[-5:]))

    # Plotting for the number of hidden units
    plt.figure(figsize=(10, 6))
    plt.plot(hidden_units_list, avg_train_entropies, label='Average Training Cross-Entropy', marker='o')
    plt.plot(hidden_units_list, avg_valid_entropies, label='Average Validation Cross-Entropy', marker='o')
    ...
    print("Running experiment for varying learning rates...")
    for lr in learning_rate_list:
        print(f"Training with learning rate {lr}...")
        _, _, train_entropies, valid_entropies = SGD(X_train, y_train, X_val, y_val, 50, num_epoch, init_flag, lr)

        # Plotting for each learning rate
        plt.figure(figsize=(10, 6))
        plt.plot(range(1, num_epoch + 1), train_entropies, label=f'Training Cross-Entropy (LR={lr})')
        plt.plot(range(1, num_epoch + 1), valid_entropies, label=f'Validation Cross-Entropy (LR={lr})', linestyle='--')
        plt.xlabel('Epochs')
        plt.ylabel('Cross-Entropy')
        plt.title(f'Cross-Entropy vs Epochs (LR={lr})')
        plt.legend()
        plt.draw()
        plt.waitforbuttonpress(0)
        plt.close()

#Defining the hidden units and learning rate parameters for the experiment
hidden_units = [5, 20, 50, 100, 200]
learning_rates = [0.1, 0.01, 0.001]

# Running the experiments the experiments
run_experiment(hidden_units, learning_rates)
